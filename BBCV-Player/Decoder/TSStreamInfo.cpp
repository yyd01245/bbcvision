...
TSstreamInfo::TSstreamInfo()
{
	...
	char videoinfPath[1024]={0};
	sprintf(videoinfPath,"videoinfo_%d_%d_%d.log",tmnow.tm_hour,tmnow.tm_min,tmnow.tm_sec);
	m_Mediafp = fopen(videoinfPath,"wb");;
	m_llFrameNum = 0;
	m_iFramTotal =	0;
	m_llFrameTotal = 0;
	m_iGopSize = 0;
...
bool TSstreamInfo::Get_tsIFrame_size(int* iSize)
{
	*iSize = m_iIframeSize;
	return true;
}

bool TSstreamInfo::Set_tsDecoder_SaveStream(bool bSaveStream)
{
	m_bSaveStream = bSaveStream;
